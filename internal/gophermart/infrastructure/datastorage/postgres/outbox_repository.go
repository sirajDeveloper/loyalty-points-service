...
type outboxRepositoryTx struct {
	tx pgx.Tx
}

func (r *outboxRepositoryTx) Create(ctx context.Context, outbox *model.Outbox) error {
	query := `INSERT INTO outbox (order_id, status, retries, created_at, updated_at) 
	          VALUES ($1, $2, $3, $4, $5) RETURNING id`
	err := r.tx.QueryRow(ctx, query, outbox.OrderID, outbox.Status, outbox.Retries, outbox.CreatedAt, outbox.UpdatedAt).Scan(&outbox.ID)
	return err
}

func (r *outboxRepositoryTx) FindPending(ctx context.Context, limit int) ([]*model.Outbox, error) {
	query := `SELECT id, order_id, status, retries, created_at, updated_at 
	          FROM outbox WHERE status = 'PENDING' 
	          ORDER BY created_at ASC LIMIT $1`
...
func (r *outboxRepositoryTx) UpdateStatus(ctx context.Context, outboxID int64, status model.OutboxStatus) error {
	query := `UPDATE outbox SET status = $1, updated_at = NOW() WHERE id = $2`
	_, err := r.tx.Exec(ctx, query, status, outboxID)
	return err
}

func (r *outboxRepositoryTx) IncrementRetries(ctx context.Context, outboxID int64) error {
	query := `UPDATE outbox SET retries = retries + 1, updated_at = NOW() WHERE id = $1`
	_, err := r.tx.Exec(ctx, query, outboxID)
	return err
}
